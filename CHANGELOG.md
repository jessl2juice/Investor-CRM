...
- **Cloud deployment** to Cloudflare Pages (frontend) + Fly.io (backend) + Neon PostgreSQL (database)
- `Dockerfile.fly` for Fly.io backend-only deployment
- `fly.toml` configuration for Fly.io (app: `bettermind-crm-api`, region: `iad`)
- `deploy-fly-backend.ps1` one-command backend deploy script
- `deploy-cf-frontend.ps1` one-command frontend build + deploy script
- `docs/CLOUD_DEPLOYMENT.md` comprehensive cloud migration guide
- `VITE_API_BASE_URL` environment variable support in `frontend/src/api.js` for dynamic backend URL
- SSL auto-detection in `database.py` for Neon PostgreSQL connections via `pg8000`
- `allow_origin_regex` in CORS middleware for localhost/127.0.0.1 on any port

### Changed

- `bettermind.buzz` DNS now points to Cloudflare Pages (was Cloudflare Tunnel)
- Architecture: from Docker Desktop + Cloudflare Tunnel ($0/mo, PC required) to cloud ($3-5/mo, 24/7)
- Fly.io machines set to `auto_stop_machines = false` with `min_machines_running = 1` to prevent cold start CORS failures
- Updated all documentation to reflect cloud deployment as primary

### Removed

- Local Docker Desktop deployment decommissioned (containers and Cloudflare Tunnel service removed)
